...
def showData(image):
    ...
    for byte in all_bytes:
        ...
        if decoded_data[(-1*len(ENDSIGNAL)):] == ENDSIGNAL : # check if we reached delimiter which is ENDSIGNAL
...
def encodeimage(image_name,data,steganofilename):
    ...
    # steganofilename = input('enter the name of new encoded image(WITHOUT extension): ...')
    steganofilename+='.png'
    encoded_image = hideData(image,data) # call the hidedata function to hide the secret message into input image
    cv2.imwrite(steganofilename,encoded_image)
...
def testimagemodule():
    while True:
        ...
        if(userinput =='1'):
            ...
            data = input('enter the string to be hiden: ....')
            steganofilename = input('enter the name of new encoded image(WITHOUT extension): ...')            
            encodeimage(image_name,data,steganofilename)
        elif(userinput =='2'):
            print('\ndecoding....')
            image_name = input('enter the name of the steganographed image type PNG that you want to decode (with extension): ... ')            
            print('\ndecoded message is:-'+ decode_text(image_name)+'-')
        else:
            break
