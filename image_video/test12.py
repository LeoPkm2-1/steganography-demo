...
def main():
    ...
    call(["ffmpeg", "-i", "tmp/%d.png" , "-vcodec", "png", "tmp/video-test-leo.mp4", "-y"],stdout=open(os.devnull, "w"), stderr=STDOUT)
    
    call(["ffmpeg", "-i", "tmp/video-test-leo.mp4", "-i", "tmp/audio.mp3", "-codec", "copy", "video-test-leo.mp4", "-y"],stdout=open(os.devnull, "w"), stderr=STDOUT)
    
       
    
    print("-----------------888-----------------")
    call(["ffmpeg", "-i", "video-test-leo.mp4", "-vcodec", "libx264", "x264-video-test-leo.mp4", "-y"],stdout=open(os.devnull, "w"), stderr=STDOUT)
    clean_tmp()
