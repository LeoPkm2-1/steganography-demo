...
def main():
    ...
    # encode_string(input_string)
    # call(["ffmpeg", "-i", "tmp/%d.png" , "-vcodec", "libx264",'-b:v','10341k','-filter:v','fps=30', "tmp/video5.mp4", "-y"],stdout=open(os.devnull, "w"), stderr=STDOUT)
    
    
    encode_string(input_string)
    call(["ffmpeg", "-i", "tmp/%d.png" , "-crf", "18",'-b:v','10341k','-filter:v','fps=30', "tmp/video6.mp4", "-y"],stdout=open(os.devnull, "w"), stderr=STDOUT)
    
    
    # call(["ffmpeg", "-i", "tmp/video.mov", "-i", "tmp/audio.mp3", "-codec", "copy", "video.mov", "-y"],stdout=open(os.devnull, "w"), stderr=STDOUT)
    # clean_tmp()
    
    call(["ffmpeg", "-i", "tmp/video6.mp4", "-i", "tmp/audio.mp3", "-codec", "copy", "video6.mp4", "-y"],stdout=open(os.devnull, "w"), stderr=STDOUT)
    clean_tmp()
